/**
 * Celebration effects module for unit system changes.
 * Handles visual and audio feedback when switching between imperial and metric units.
 */
...
/**
 * Ensures the celebration stylesheet is loaded before running any visual effects.
 * Returns a promise that resolves when the stylesheet is ready.
 */
...
/**
 * Removes the confetti animation from the DOM and clears any pending cleanup timers.
 */
...
/**
 * Launches a confetti animation with randomly positioned and animated pieces.
 * The confetti will automatically clean itself up after the animation completes.
 */
...
/**
 * Removes the eagle animation from the DOM, stops the audio, and cleans up confetti.
 */
...
/**
 * Triggers the "freedom eagle" celebration effect with animation, sound, and confetti.
 * Used when switching to imperial units.
 */
export function triggerFreedomEagle() {
  destroyEagle();
...
/**
 * Dismisses any active alert and clears pending auto-dismiss timers.
 */
...
/**
 * Shows a temporary alert when switching to metric units.
 * The alert will automatically dismiss after a few seconds.
 */
export function showFreedomAlert() {
  dismissAlert();
  destroyEagle();
...
/**
 * Handles the appropriate celebration effect based on the current unit system.
 * @param {string} units - The current units ('in' or 'mm')
 */
